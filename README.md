...
A [preprint](https://arxiv.org/abs/2304.13833) is on arXiv. The code was tested under Python `3.11.1` and NumPy `1.24.1`. To run a batch of experiments, execute `experiments.py`. You may want to change some parameters (e.g. training samples, MCMC iterations, etc.) at the top of the script. Results are saved as `.pickle` files and stored in `experiments` directory. To process the results, execute `analysis.py`.
